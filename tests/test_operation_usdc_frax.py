...
def test_usdc_frax_fresh_convex(usdc, stratms, whale, Strategy, strategy_usdc_frax, accounts, fraxyvault, chain, usdc_vault, live_usdc_vault, ychad, gov, strategist, interface):
    ...
    fraxcrvStrat1 = Strategy.at(yvault.withdrawalQueue(0))
    fraxcrvStrat2 = Strategy.at(yvault.withdrawalQueue(1))
    vault.addStrategy(strategy, 0, 0, 2**256-1, 1000, {"from": gov})
    strategy.updateMinTimePerInvest(0,{'from':gov})
    ssc_ib = Strategy.at("0x0c8f62939Aeee6376f5FAc88f48a5A3F2Cf5dEbB")
    vault.updateStrategyDebtRatio(ssc_ib, 0, {"from": gov})
    vault.updateStrategyDebtRatio(strategy, 1_000, {"from": gov})
    tx = ssc_ib.harvest({'from': gov})
    ...
    tx = fraxcrvStrat2.harvest({"from": gov})
    chain.sleep(2016000)
    chain.mine(1)
    
    genericStateOfStrat(strategy, currency, vault)
    convex_deposit.earmarkRewards(32, {"from": gov})
    tx = fraxcrvStrat1.harvest({"from": gov})
    print("fraxcrvStrat1",tx.events["Harvested"])
    chain.sleep(2016000)
    chain.mine(1)
    # ibcrvStrat1.harvest({"from": gov})
    convex_deposit.earmarkRewards(29, {"from": gov})
    tx = fraxcrvStrat2.harvest({"from": gov})
    print("fraxcrvStrat2",tx.events["Harvested"])
    chain.sleep(2016000)
    chain.mine(1)
    tx = strategy.harvest({'from': gov})
