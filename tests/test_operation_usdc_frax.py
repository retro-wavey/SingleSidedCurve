...
def test_usdc_ib_fresh_convex(usdc, stratms, whale, Strategy, strategy_usdc_frax, accounts, fraxyvault, chain, usdc_vault, live_usdc_vault, ychad, gov, strategist, interface):
    ...
    tx = fraxcrvStrat1.harvest({"from": vGov})
    print("fraxcrvStrat1",tx.events["Harvested"])
    chain.sleep(2016000)
    chain.mine(1)
    # ibcrvStrat1.harvest({"from": vGov})
    convex_deposit.earmarkRewards(29, {"from": vGov})
    tx = fraxcrvStrat2.harvest({"from": vGov})
